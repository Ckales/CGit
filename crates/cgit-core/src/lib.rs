...
pub fn start_watching(
    state: &WatchState,
    path: &str,
    on_change: impl Fn(&str) + Send + 'static,
) -> Result<(), String> {
        ...
        match item {
            ...
            gix::status::Item::IndexWorktree(change) => {
                ...
                let (file, status) = match &change {
                    ...
                    Item::DirectoryContents { entry, .. } => (entry.rela_path.to_string(), "new"),
...
pub fn get_branch_tracking(path: String) -> Result<Tracking, String> {
    let repo = open(&path)?;
...
pub fn get_tags(path: String) -> Result<Vec<String>, String> {
    ...
    for t in refs.tags().map_err(|e| e.to_string())? {
        ...
        let time = repo.find_tag(t.id()).ok().and_then(|tag| {
            tag.tagger()
                .ok()
                .flatten()
...
pub fn push_tag(path: String, name: String) -> Result<String, String> {
    run_git(&path, &["push", "origin", name.as_str()])
}
...
pub fn get_blame(path: String, file: String) -> Result<Vec<BlameLine>, String> {
    let repo = open(&path)?;
    let head = repo
        .head_commit()
    ...
    for (i, text) in content.lines().enumerate() {
        ...
        if let Some(id) = by_line.get(&(i as u32)) {
            ...
            let (author, summary) =
                meta.entry(*id)
                    .or_insert_with(|| match repo.find_commit(*id) {
                        Ok(c) => (
...
pub fn clone_repo(url: String, dir: String, on_progress: impl Fn(&str)) -> Result<String, String> {
    use std::io::Read;
...
pub fn unstage_file(path: String, file: String) -> Result<(), String> {
    ...
    if repo.head_commit().is_ok() {
        ...
        run_git(
            &path,
            &["rm", "--cached", "--force", "--quiet", "--", file.as_str()],
        )?;
    }
    Ok(())
}
...
pub fn get_commit_files(path: String, oid: String) -> Result<Vec<FileStatus>, String> {
    let repo = open(&path)?;
...
pub fn get_push_files(path: String) -> Result<Vec<FileStatus>, String> {
    let repo = open(&path)?;
...
pub fn get_hunks(path: String, file: String, staged: bool) -> Result<Hunks, String> {
...
pub fn get_rebase_todo(path: String, base: String) -> Result<Vec<TodoCommit>, String> {
    let range = format!("{}..HEAD", base);
    let out = run_git(
        &path,
        &["log", "--reverse", "--format=%H%x09%s", range.as_str()],
    )?;
    let mut v = Vec::new();
    for line in out.lines() {
...
pub fn get_git_credential(path: String) -> Result<GitCredentialInfo, String> {
...
pub fn get_identity(path: String) -> Result<Identity, String> {
    let repo = open(&path)?;
...
pub fn update_branch(path: &str, name: &str) -> Result<String, String> {
    let repo = open(path)?;
...
pub fn push_branch(path: &str, name: &str) -> Result<String, String> {
    let repo = open(path)?;
...
pub fn git_push_force(path: String) -> Result<String, String> {
    ...
    run_git(
        &path,
        &["push", "--force-with-lease", "origin", branch.as_str()],
    )
}
...
mod tests {
    ...
    fn saving_credentials_uses_the_configured_git_helper() {
        ...
        let info = save_git_credential(&path, "Ckales", "test-token").unwrap();
    ...
    fn a_merge_commit_refuses_instead_of_exporting_its_parent() {
        ...
        let merge = run_git(&path, &["rev-parse", "HEAD"])
            .unwrap()
            .trim()
            .to_string();
        ...
        let normal = run_git(&path, &["rev-parse", "HEAD^"])
            .unwrap()
            .trim()
            .to_string();
    ...
    fn temp_clone(name: &str) -> String {
        ...
        run_git(
            &root,
            &["init", "-q", "--bare", "-b", "master", "remote.git"],
        )
        .unwrap();
    ...
    fn a_branch_updates_and_pushes_without_being_checked_out() {
        ...
        update_branch(&work, "other").unwrap();
        ...
        assert_eq!(
            current_branch(&open(&work).unwrap()).as_deref(),
            Some("master")
        );
    ...
    fn curl_config_escapes_the_json_body() {
        ...
        assert!(
            config.contains(r#"data-binary = "{\"a\":\"say \\\"hi\\\"\""#),
            "{config}"
        );
        assert!(
            config.contains(r#"header = "Authorization: Bearer tok\"en""#),
            "{config}"
        );
    }
    ...
    fn status_sides_are_independent() {
        ...
        let rows = get_status(repo.clone()).unwrap();
        ...
        run_git(&repo, &["add", "f"]).unwrap();
        let rows = get_status(repo).unwrap();
        let f: Vec<_> = rows.iter().filter(|r| r.path == "f").collect();
